...
const entries: Prisma.EntryCreateInput[] = [
  {
    ...
    links: [
      "https://github.com/solana-labs/solana/blob/master/core/src/banking_stage.rs",
    ],
  ...
  {
    term: "Transaction Process Unit (TPU)",
    definition:
  ...
  {
    term: "United States Dollar Coin (USDC)",
    definition: "A stable coin that has the same value as the US Dollar.",
    links: ["https://www.circle.com/en/usdc"],
    tags: [],
    longDefinition:
  ...
  },
  {
    ...
    links: ["https://docs.solanalabs.com/consensus/turbine-block-propagation"],
    tags: [],
    longDefinition:
...
export const seed = protectedProcedure.mutation(async ({ ctx }) => {
  ...
  await Promise.all(
    entries.map(async ({ term, definition, links, tags, longDefinition }) => {
      const entry = await ctx.db.entry.create({
        data: { term, definition, links, tags, longDefinition },
      });
